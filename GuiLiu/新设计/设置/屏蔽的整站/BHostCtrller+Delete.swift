...
extension BHostCtrller {
    func deleteBlockedHost(_ item: BlockedHost){
        //检测
        ...
        url.appendPathComponent("deleteblockedhost")
        ...
        let task = URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            case 200:
                OperationQueue.main.addOperation {
                    ...
                    guard let index = self.listArray.index(of: item) else {
                        debugPrint("Can't find BHost index")
                        return
                    }
            ...
            default:
                if let dataT = data,let json = try? JSONSerialization.jsonObject(with: dataT), let jsonD = json as? Dictionary<String,Any>,let reason = jsonD["reason"] as? String {
                    ...
                    GPresentWarning("删除网站Host过滤条目接口中无法识别的错误")
                }
            }
        }
        task.resume()
